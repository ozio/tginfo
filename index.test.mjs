...
const userSlug = 'mr_ozio'
const userWebURL = `${TG_DOMAIN}/${userSlug}`
const userTgURL = `tg://resolve?domain=${userSlug}`
const userSlugAt = `@${userSlug}`
const channelPreviewURL = `${TG_DOMAIN}/s/${userSlug}`
const inviteCode = 'iNviTeC0De-12345'
const inviteSlug = `+${inviteCode}`
...
it('Should convert anything to web URL', async () => {
  ...
  const b1 = await tginfo(inviteSlug, ['webUrl'])
